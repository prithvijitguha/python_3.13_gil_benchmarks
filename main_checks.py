"""This program is to test benchmarks for Python with gil and no gil,
it runs a set of regular single threaded checks, multithreading and then multiprocessing

Args:
    PYTHON_GIL: A global variable to check for gil lock enabled/disabled"""
...
def is_prime(n: int) -> bool:
    """Function to check if an integer provided to it is prime or not

    Args:
        n: An input integer to check

    Returns:
        result: Bool of true or false, True if prime and False if not prime"""
    if n <= 1:
        return False
...
def count_primes(start: int, end: int) -> int:
    """Iterate from start to finish and find out how many prime numbers are
    present within a range"""
    count = 0
...
def threaded_count_primes(n: int, num_threads: int) -> int:
    """Function to use multithreading to count the number of prime numbers"""
    threads = []
...
def multiprocess_count_primes(n: int, num_processes: int) -> int:
    """Function to use multiprocessing to count the number of prime numbers"""
